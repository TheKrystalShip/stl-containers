...
namespace TKS::STL
{
    ...
    template <typename T>
    inline typename CircularBuffer<T>::size_type
    ...
    template <typename T>
    inline typename CircularBuffer<T>::reference
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_reference
    CircularBuffer<T>::front() const
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        if (empty())
            throw std::length_error("front function called on empty buffer");
    ...
    template <typename T>
    inline void CircularBuffer<T>::push_back(const value_type &data)
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        // if(full())
        //	_buff[_tail].~T();
    ...
    template <typename T>
    inline void CircularBuffer<T>::push_back(value_type &&data) noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
    ...
    template <typename T>
    inline void CircularBuffer<T>::_increment_bufferstate()
    {
        if (full())
    ...
    template <typename T>
    inline void CircularBuffer<T>::_decrement_bufferstate()
    {
        --_size;
        _tail = (_tail + 1) % _max_size;
    }
    ...
    template <typename T>
    inline typename CircularBuffer<T>::reference
    CircularBuffer<T>::operator[](size_t index)
    {
        std::lock_guard<std::mutex> _lck(_mtx);
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_reference
    CircularBuffer<T>::operator[](size_t index) const
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        if ((index < 0) || (index >= _size))
            throw std::out_of_range("Index is out of Range of buffer size");
    ...
    template <typename T>
    inline typename CircularBuffer<T>::reference
    CircularBuffer<T>::at(size_t index)
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        if ((index < 0) || (index >= _size))
            throw std::out_of_range("Index is out of Range of buffer size");
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_reference
    CircularBuffer<T>::at(size_t index) const
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        if ((index < 0) || (index >= _size))
            throw std::out_of_range("Index is out of Range of buffer size");
    ...
    template <typename T>
    inline typename CircularBuffer<T>::iterator
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_iterator
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_iterator
    CircularBuffer<T>::cbegin() const noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        const_iterator iter;
        iter._ptrToBuffer = this;
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_iterator
    CircularBuffer<T>::cend() const noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        const_iterator iter;
        iter._ptrToBuffer = this;
    ...
    template <typename T>
    inline typename CircularBuffer<T>::iterator
    CircularBuffer<T>::rbegin() noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        iterator iter;
        iter._ptrToBuffer = this;
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_iterator
    CircularBuffer<T>::rbegin() const noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        const_iterator iter;
        iter._ptrToBuffer = this;
    ...
    template <typename T>
    inline typename CircularBuffer<T>::iterator
    CircularBuffer<T>::rend() noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        iterator iter;
        iter._ptrToBuffer = this;
    ...
    template <typename T>
    inline typename CircularBuffer<T>::const_iterator
    CircularBuffer<T>::rend() const noexcept
    {
        std::lock_guard<std::mutex> _lck(_mtx);
        const_iterator iter;
        iter._ptrToBuffer = this;
